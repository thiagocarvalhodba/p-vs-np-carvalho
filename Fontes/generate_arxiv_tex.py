...
Fully updated with all minor revisions from the Annals / STOC senior review panel:
1. Definition of Admissible 3-CNF Formulas (H1, H2, H3)
2. Asymptotic Drift Flow Remark (M_spur >> (1/3)^N)
3. Convexity vs Discrete Integrality Barrier Remark
4. Precision on Morse Index and Strong Minimum Principle
"""
...
\begin{definition}[Admissible 3-CNF Formulas]
\label{def:admissible}
A 3-CNF formula $F$ is called \textbf{admissible} if it satisfies:
\begin{enumerate}
    \item \textbf{(H1 - Non-Tautology):} No clause contains both a literal and its negation ($\ell_j \neq \neg \ell_k$).
    \item \textbf{(H2 - Variable Connectivity):} Every variable $x_i$ appears in at least one clause ($\deg(x_i) \geq 1$).
    \item \textbf{(H3 - Non-Constant Discrete Energy):} The discrete clause error $E_{\rm disc}(s)$ is not identically constant on $\{-1, +1\}^N$, ensuring the existence of at least one violating assignment ($E_{\rm disc}(s^*) \geq 1$).
\end{enumerate}
\end{definition}
...
For any admissible 3-CNF formula $F$ over $N$ variables, the spurious critical set of the quadratic hinge relaxation contains the open central hypercube $\mathcal{U}_N = (-1/3, 1/3)^N$. Consequently, its Lebesgue measure satisfies:
\begin{equation}
...
By (H3), there exists at least one discrete Boolean assignment $s^* \in \{-1, +1\}^N$ violating at least one clause ($E_{\rm disc}(s^*) \geq 1$). The associated open orthant $\Omega_{s^*} = \mathcal{U}_N \cap \{x \mid \text{sign}(x) = s^*\}$ has measure $\mu(\Omega_{s^*}) = (1/3)^N > 0$. For all $x \in \Omega_{s^*}$, $\nabla \Phi_{\rm quad}(x) = \mathbf{0}$ and $E_{\rm disc}(\text{sign}(x)) \geq 1$. For UNSAT formulas, all $2^N$ orthants satisfy this condition, proving the theorem.
\end{proof}
...
\begin{remark}[Asymptotic Drift Flow and Inscribed Core]
\label{rem:drift_flow}
While $\mu(\mathcal{U}_N) \geq (1/3)^N$ vanishes exponentially as $N \to \infty$, the hypercube $\mathcal{U}_N$ represents merely the strictly inscribed central core of the full LP feasible polytope $\mathcal{P}_{\rm LP} = \{x \in [-1, 1]^N \mid \forall c, g_c(x) \leq 0\}$, throughout which $\Phi_{\rm quad} \equiv 0$ and $\nabla \Phi_{\rm quad} \equiv \mathbf{0}$.
In random 3-SAT ensembles near the critical threshold $\alpha_c$, statistical symmetry of literal polarities induces a net inward restoring drift field outside $\mathcal{P}_{\rm LP}$:
\begin{equation}
\mathbb{E}_F \left[ -\nabla_i \Phi_{\rm quad}(x) \right] \approx -\kappa(\alpha) x_i \quad (\kappa(\alpha) > 0).
\end{equation}
This drift field funnels trajectories originating in the periphery of $[-1, 1]^N$ directly toward $\mathcal{P}_{\rm LP}$. Trajectories freeze permanently upon hitting the boundary $\max_c g_c(x) \leq 0$, explaining why the empirical spurious attractor mass satisfies $\mathcal{M}_{\rm spur}(\Phi_{\rm quad}) \approx 0.96$ at $N=60$ despite $\mu(\mathcal{U}_N) \to 0$.
\end{remark}
...
For any admissible 3-CNF formula $F$, the critical zero-sets of the multilinear and Softplus relaxations have Lebesgue measure strictly zero:
\begin{equation}
...
\begin{proof}
\textbf{Case 1: Multilinear ($\Phi_{\rm mult}$).} The potential $\Phi_{\rm mult}(x)$ is a polynomial in $\mathbb{R}[x_1, \dots, x_N]$. By (H3), $E_{\rm disc}(s)$ is not identically constant on $\{-1, +1\}^N$. Since $\Phi_{\rm mult}|_{\{-1, +1\}^N} = E_{\rm disc}$, the polynomial $\Phi_{\rm mult}$ cannot be constant on $\mathbb{R}^N$. By elementary multivariate algebra, a polynomial is constant if and only if all its partial derivatives vanish identically. Hence, there exists an index $k \in \{1, \dots, N\}$ such that $P_k(x) = \partial_k \Phi_{\rm mult}(x) \not\equiv 0$. By the Real Polynomial Root Measure Lemma \cite{okamoto1973distinctness, caron2005zero}, the zero set $Z(P_k) = \{x \in \mathbb{R}^N \mid P_k(x) = 0\}$ of a non-zero real polynomial has Lebesgue measure zero: $\mu(Z(P_k)) = 0$. Since $\text{Crit}(\Phi_{\rm mult}) \subseteq Z(P_k)$, it follows that $\mu(\mathcal{C}_0(\Phi_{\rm mult})) \leq \mu(Z(P_k)) = 0$.

\textbf{Case 2: Softplus ($\Phi_{\rm soft}$).} The function $\Phi_{\rm soft}(x)$ is real analytic ($\mathcal{C}^\omega$) on the connected domain $\mathbb{R}^N$. Since $\Phi_{\rm soft} \not\equiv \text{const}$, there exists $k$ such that $\partial_k \Phi_{\rm soft} \not\equiv 0$. By the Identity Theorem for Real Analytic Functions \cite{krantz2002primer}, the zero set of a non-identically zero real analytic function on a connected domain has Lebesgue measure zero: $\mu(Z(\partial_k \Phi_{\rm soft})) = 0$. Hence $\mu(\mathcal{C}_0(\Phi_{\rm soft})) = 0$.
\end{proof}
...
\label{thm:harmonic}
For any 3-CNF formula $F$ satisfying (H1), the multilinear potential $\Phi_{\rm mult}$ has identically vanishing Laplacian everywhere on $\mathbb{R}^N$:
\begin{equation}
...
\begin{proof}
For any 3-SAT clause $c = (\ell_1 \lor \ell_2 \lor \ell_3)$, the variables are distinct by (H1). The clause term $\phi_c(x) = \prod_{j \in c} \frac{1 - \sigma_j^{(c)} x_j}{2}$ is of degree at most 1 in each coordinate $x_i$. Differentiating twice with respect to the same variable:
\begin{equation}
...
Under the projected gradient flow on the compact hypercube $\mathcal{X} = [-1, 1]^N$, \textbf{every isolated local minimum of $\Phi_{\rm mult}$ is confined exclusively to the $2^N$ discrete vertices $\{-1, +1\}^N$} (faces of dimension $d=0$).
\end{theorem}
...
Along the edges ($d=1$), the restricted potential is strictly linear ($a + bt$), which attains its extrema exclusively at the boundary endpoints $t = \pm 1$. Therefore, all isolated local attractors of the projected dynamical flow are strictly localized at the zero-dimensional vertices $\{-1, +1\}^N$.
\end{proof}
...
\begin{remark}[Convexity vs. Discrete Integrality Gap]
\label{rem:convex_gap}
Although $\Phi_{\rm soft}$ is globally positive semidefinite ($\nabla^2 \Phi_{\rm soft} \succeq 0$), this does not imply polynomial-time tractability of discrete 3-SAT. The unconstrained continuous minimizer $x^* = \arg\min \Phi_{\rm soft}(x)$ is inherently fractional, situated near $\mathbf{0}$ for balanced formulas where $\Phi_{\rm soft}(\mathbf{0}) \approx \frac{M}{\beta}\ln(1 + e^{-\beta/2})$. Because continuous minimization does not commute with discrete rounding ($\min_s E_{\rm disc}(s) \neq \text{round}(\min_x \Phi_{\rm soft}(x))$), rounding the continuous minimizer incurs the combinatorial integrality gap. Forcing continuous minimizers to discrete vertices requires $\beta \to \infty$, which triggers the exponential curvature collapse, algorithmic stiffness ($L_\beta = \Theta(\beta)$), and floating-point underflow formalized in Theorem~\ref{thm:bifurcation}.
\end{remark}
...
We define the \textbf{Spurious Attractor Mass} via the $\omega$-limit set as:
\begin{equation}
\mathcal{M}_{\rm spur}(\Phi) \equiv \mu\left( \left\{ x_0 \in \mathcal{X} \;\middle|\; \omega(x_0) \subseteq \mathcal{S}_{\rm spur}(\Phi) \right\} \right).
\end{equation}
...
print(f"Refined LaTeX Monograph successfully generated: {tex_path}")

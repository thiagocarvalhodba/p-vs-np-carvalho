...
\begin{enumerate}
    ...
    \end{equation}
    Furthermore, no projected equilibria exist with $\Phi_{\rm quad}(x^*) > 0$: any stationary point on the boundary $\partial \mathcal{X}$ satisfies $-\nabla \Phi_{\rm quad}(x^*) \in N_{\mathcal{X}}(x^*)$, which would require $\langle -\nabla \Phi_{\rm quad}(x^*), x^* \rangle \ge 0$, directly contradicting strict centripetal contraction. By LaSalle's Invariance Principle, every trajectory must converge to $Z(\nabla \Phi_{\rm quad})$.
\end{enumerate}
\end{theorem}

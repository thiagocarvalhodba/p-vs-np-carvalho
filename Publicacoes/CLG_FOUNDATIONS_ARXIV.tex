...
\begin{enumerate}
    ...
    \end{equation}
    Since the dependency graph is an acyclic DAG, the Jacobian is strictly upper triangular under topological ordering, defining a monotone cooperative cascade system \cite{smith1995monotone}. By induction along the topological order, the projected gradient flow converges monotonically to satisfying models from almost all initial conditions:
    \begin{equation}
    \mathcal{M}_{\rm spur}(\Phi_{\rm mult}) = o(1).
    \end{equation}
    \item Simultaneously, for any chain of implications of length $K = \Omega(N)$, $x_1 \to x_2 \to \dots \to x_K$, the central box $\mathcal{U}_N = (-1/3, 1/3)^N \subset Z$ has identically vanishing gradient $\nabla \Phi_{\rm quad} \equiv 0$. Under coordinate-wise Boolean rounding $\text{sign}(x)$, each variable is independently $+1$ or $-1$ with probability $1/2$. The probability that all $K-1$ clauses are satisfied is $(3/4)^{K-1} \to 0$ as $K \to \infty$. By Theorem~\ref{thm:universal_contraction}, all trajectories outside $Z$ converge to $Z$ via LaSalle invariance, yielding almost sure violation upon rounding:
    \begin{equation}
    \mathcal{M}_{\rm spur}(\Phi_{\rm quad}) \ge 1 - (3/4)^{K-1} = 1 - o(1).
    \end{equation}
\end{enumerate}
...
\begin{theorem}[Theorem 10: Rigorous Separation in Leaf-Decoupled Acyclic Hypertrees and Subcritical 3-SAT]
\label{thm:subcritical_separation}
Let $\mathcal{H}$ be an acyclic 3-CNF hyperforest where each clause contains at least one free leaf variable (leaf-decoupled hypertrees), which characterizes the connected components in random 3-SAT below the hypergraph percolation threshold $\alpha < \alpha_c = 1/6$ \cite{schmidt1985component, karonski2002random}:
\begin{enumerate}
    \item In any leaf-decoupled hypertree, leaf-to-root induction proves that every assignment with positive discrete energy ($E_{\rm disc} > 0$) admits a flip of an active leaf variable that strictly decreases energy without affecting any other clause. Consequently, no discrete local minima with $E_{\rm disc} > 0$ exist.
    \item By Theorem~\ref{thm:vertex_geom}, all local minima of $\Phi_{\rm mult}$ in faces inherit vertex values, precluding local minima with positive potential. Every non-satisfying critical point is a strict saddle with $\lambda_{\min}(\nabla^2 \Phi_{\rm mult}) < 0$.
    ...
    \end{equation}
    \item For $\Phi_{\rm quad}$, by Theorem~\ref{thm:universal_contraction} every trajectory converges to the LP polytope $Z$, which deterministically contains the central box $\mathcal{U}_N$ with volume $\ge (1/3)^N$. Points in $\mathcal{U}_N$ undergo unbiased Boolean rounding, failing each clause with probability $1/8$, which yields strictly positive residual violation density:
    \begin{equation}

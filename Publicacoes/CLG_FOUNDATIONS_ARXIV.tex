...
Let $M=\lfloor\alpha N\rfloor$ with fixed $\alpha>0$, initialize the
projected multilinear gradient flow uniformly on $[-1,1]^N$, and round its
limit coordinatewise to a Boolean assignment.  In the uniform model of
distinct signed clauses,
\begin{equation}
\liminf_{N\to\infty}\mathbb E[\rho_{\rm mult}(\alpha)]
\ge \frac{729}{2^{59}}\alpha^5e^{-39\alpha}>0.
\label{eq:t10_refutation}
\end{equation}
The independent-clause model has the same leading constant.  Hence the former
claim $\rho_{\rm mult}(\alpha)\to0$ in expectation, and therefore the former
``complete subcritical separation'' theorem for $0<\alpha<1/6$, is false.
\end{theorem}

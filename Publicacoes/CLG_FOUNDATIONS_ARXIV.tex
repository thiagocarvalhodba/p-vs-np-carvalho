...
(8) We prove analytically that the normalized volume of the LP polytope in random 3-SAT satisfies the finite-dimensional lower bound $\mathbb{E}[\mu_{\rm norm}(Z)] \ge (5/6)^{\alpha N} = e^{-N \alpha \ln(6/5)} > 0$ via the Irwin-Hall distribution of order 3;
(9) We analyze dynamical behavior in monotone Horn families, showing that multilinear gradient flow converges to zero-energy models via Hirsch cooperativity, while demonstrating that Hinge separation requires general DAG structures due to the singleton collapse under positive unit facts;
...
\begin{enumerate}
    \item Under $\Phi_{\rm mult}$, the cross-derivatives along edges are strictly non-negative: $J_{ij}(x) = -\frac{\partial^2 \Phi_{\rm mult}}{\partial x_i \partial x_j} = +\frac{1}{4} \ge 0$, defining a cooperative dynamical system (Hirsch 1985). By the absence of competing bodies in linear clauses, the flow converges to a satisfying model with zero residual energy from almost all initial conditions: $\mathcal{M}_{\rm spur}(\Phi_{\rm mult}) = o(1)$.
    \item \textbf{Status of Dynamic Separation:} While $\Phi_{\rm mult}$ successfully reaches satisfying models, characterizing the exact spurious basin mass $\mathcal{M}_{\rm spur}(\Phi_{\rm quad})$ for general Horn DAGs with competing clauses remains an \textbf{Open Problem}. The proof cannot rely on single linear chains with head facts due to the singleton collapse $Z = \{(+1, \dots, +1)\}$.
\end{enumerate}
\end{theorem}
...
\textbf{Dynamical Separation} & Trapped in LP Polytope $Z$ & Horn / Subcritical: $\rho_{\rm mult}=0$ (T9/T10 Open) & Regularized finite ($\beta < \infty$) \\
\bottomrule
\end{tabular}
\label{tab:synthesis}
\end{table}

...
\end{enumerate}
Therefore, $\mathcal{E}_{\rm proj} \equiv Z$ holds identically on $\mathcal{X}$. Since the energy dissipation satisfies $\dot{\Phi}_{\rm quad}(x) = -\|\Pi_{T_{\mathcal{X}}(x)}(-\nabla \Phi_{\rm quad}(x))\|_2^2 \le 0$, by LaSalle's Invariance Principle all trajectories converge to the largest invariant set contained in $\{\dot{\Phi}_{\rm quad} = 0\} = \mathcal{E}_{\rm proj}$, which is identically $Z$.
\end{proof}
...
\begin{proposition}[Proposition 7A: Jacobian Structure for Purely Negative Clause Families (Under Re-Audit)]
...
\begin{enumerate}
    ...
    \end{equation}
    The multilinear gradient flow $\dot{x} = -\nabla \Phi_{\rm mult}(x)$ is therefore strictly competitive/inhibitory. It violates the Kamke-M\"uller condition ($J_{ij} \ge 0$) required for cooperative/monotone dynamical systems, precluding any direct invocation of Hirsch's monotone convergence theorems.
    \item \textbf{Hinge Spurious Basin:} For the quadratic Hinge relaxation $\Phi_{\rm quad}$, the open domain $A_N = (1/3, 1)^N$ converges under projected gradient descent to the LP polytope $Z$ with non-negative coordinates ($x_i(t) \ge 0$), yielding spurious Boolean rounding $\text{sign}(x) = (+1, \dots, +1)$ with positive basin mass $\mathcal{M}_{\rm spur}(\Phi_{\rm quad}) \ge (1/3)^N > 0$.
    \item \textbf{Audit Status:} The dynamical characterization of the multilinear flow on $F_N$ is formally suspended under active structural re-audit.
\end{enumerate}
\end{proposition}

\begin{remark}[Analytical Audit Note on Proposition 7A]
The derivation confirms that interactions between variables in negative clauses exert mutual competitive suppression ($J_{ij} \le 0$). Analyzing convergence on such competitive systems requires distinct invariant manifold methods rather than order-preserving flows.
\end{remark}
...
Theorem 7B (Centripetal Contraction) & \textbf{Near Closed} ($\mathcal{E}_{\rm proj} \equiv Z$, normal cone conventions explicit) \\
Proposition 7A (Negative Clause Families) & \textbf{Under Re-Audit} (Competitive Jacobian sign $J_{ij} \le 0$) \\

...
(10) We analyze subcritical random 3-SAT ($\alpha < 1/6$), establishing strict saddle avoidance for the multilinear relaxation in faces of dimension $d \ge 2$ conditioned on leaf structures ($H_{\rm leaf}$), classifying the dynamics on 1-dimensional edges, and formalizing the open status of global dynamical separation against Hinge.
...
\begin{lemma}[Lemma 10.1: Subcritical Hypertree Overlap Bound and Peeling --- Partially Closed]
\label{lem:peeling}
For random 3-SAT below the hypergraph percolation threshold $\alpha < \alpha_c = 1/6$:
\begin{enumerate}
    \item The expected number of clause pairs sharing $\ge 2$ variables satisfies the first-moment bound:
    \begin{equation}
    \mathbb{E}\left[\#\{c \ne c' \mid |c \cap c'| \ge 2\}\right] \le \binom{M}{2} \frac{18(N-3)}{N(N-1)(N-2)} \le 9 \alpha^2 < \frac{1}{4}.
    \end{equation}
    The first-moment bound demonstrates that the expected number of clause pairs sharing at least two variables is bounded by $9\alpha^2 = \mathcal{O}(1)$. The rigorous decomposition into a core of strictly linear components plus a set of $\mathcal{O}_{\mathbb{P}}(1)$ defects remains subject to additional structural hypergraph analysis.
    \item The 2-core of hyperedges is asymptotically almost surely empty ($\mathbb{P}(2\text{-core} = \emptyset) \to 1$ as $N \to \infty$), since $\alpha < 1/6 \ll \alpha_{\rm core} \approx 0.8183$.
    \item Conditioned on the linear hyperforest structure ($|c \cap c'| \le 1$), the leaf-peeling algorithm $\mathcal{A}_{\rm peel}$ terminates, inducing an elimination ordering $\pi = (c_1, \dots, c_M)$ where each leaf clause $c_t$ contains at least two private variables of global degree 1 (appearing in no other clause of the formula). Flipping a private degree-1 variable of a violated leaf strictly reduces $E_{\rm disc}$ without violating any other clause, establishing the absence of discrete local minima with $E_{\rm disc} > 0$ under this structural hypothesis.
\end{enumerate}
\end{lemma}
...
\begin{theorem}[Theorem 10: Multilinear Saddle Avoidance and Subcritical Status --- Not Closed]
\label{thm:subcritical_separation}
For the random 3-SAT ensemble $\mathcal{E}(N, \alpha)$ with $\alpha < 1/6$:
\begin{enumerate}
    \item \textbf{Classification across Face Dimensions under $H_{\rm leaf}$:}
    \begin{itemize}
        \item \textit{Faces of dimension $d \ge 2$:} By Lemma~\ref{lem:strict_saddle}, all non-satisfying relative critical points are strict saddles ($\lambda_{\min} < 0$).
        \item \textit{Faces of dimension $d = 0$ (Vertices):} By Lemma~\ref{lem:peeling}, no positive Boolean local minima exist.
        \item \textit{Faces of dimension $d = 1$ (Edges):} Restricted to any edge $\mathcal{F}$, $\Phi_{\rm mult}(t) = a t + b$ is affine. If $a \ne 0$, the edge contains no interior relative critical points (the constant non-zero gradient drives trajectories into vertices). If $a = 0$, $\nabla_{\mathcal{F}} \Phi_{\rm mult} \equiv 0$, rendering the entire open edge a degenerate flat critical segment where $\Phi_{\rm mult}(x) = E_{\rm disc}(v_1) = E_{\rm disc}(v_2)$. Such degenerate flat manifolds are not covered by strict-saddle evasion theorems.
    \end{itemize}
    \item \textbf{Conditional Strict-Saddle Evasion:} On components where all non-satisfying equilibria are strict saddles and there exist no degenerate flat critical manifolds of positive basin measure, standard stable manifold evasion results \cite{lee2019first, panageas2017gradient} imply almost sure evasion of these non-satisfying equilibria. However, establishing that degenerate flat critical segments ($d=1, a=0$) have basin measure zero or transverse instability remains an open problem; hence, concluding almost sure convergence to zero-energy models ($\lim_{N \to \infty} \rho_{\rm mult}(\alpha) = 0$) is not closed and remains strictly conjectural.
    \item \textbf{Status of Separation:} While Theorem~\ref{thm:universal_contraction} establishes that Hinge trajectories converge to the LP polytope $Z$, proving an analytical lower bound $\lim_{N \to \infty} \rho_{\rm quad}(\alpha) \ge c(\alpha) > 0$ requires a standalone subcritical basin analysis (which cannot be inherited from Theorem 8 or Theorem 9), and resolving the $d=1$ flat edge degeneracy remains necessary. Consequently, Theorem 10 is classified as \textbf{Not Closed / Under Active Investigation}.
\end{enumerate}
\end{theorem}
...
\textbf{Dynamical Separation} & Trapped in LP Polytope $Z$ & Horn / Subcritical: Open (T9 Falsified/Open, T10 Not Closed) & Regularized finite ($\beta < \infty$) \\
\bottomrule
\end{tabular}
\label{tab:synthesis}
\end{table}
...
Lemma 10.1 (Hypertree Overlap Bound) & \textbf{Partially Closed} ($9\alpha^2 = \mathcal{O}(1)$, structural decomposition pending) \\
Lemma 10.2 (Strict Saddle Subcritical) & \textbf{Closed conditionally under structural leaf hypothesis $H_{\rm leaf}$} \\
Theorem 10 (Subcritical 3-SAT Separation) & \textbf{Not Closed} ($d=1$ flat edges and Hinge separation open) \\

...
\begin{enumerate}
    ...
    \end{equation}
    Because $9\alpha^2 = \mathcal{O}(1)$ is a constant independent of $N$ (e.g., $9(0.12)^2 = 0.1296$ at $\alpha = 0.12$), Markov's inequality yields $\mathbb{P}(X \ge 1) \le 9\alpha^2$, but does not imply $\mathbb{P}(X = 0) \to 1$. Therefore, strict linearity across all components is not established a.a.s. by this first moment alone; almost all components are linear hypertrees, but strict global linearity remains conditional.
...
\begin{enumerate}
    \item Under the structural hypothesis $H_{\rm leaf}$, by Lemmata~\ref{lem:peeling} and \ref{lem:strict_saddle}, all non-satisfying relative critical points of $\Phi_{\rm mult}$ in faces of dimension $d \ge 2$ are strict saddles ($\lambda_{\min} < 0$), and no discrete positive local minima exist.
    \item By the constrained Stable Manifold Theorem for projected dynamical systems \cite{lee2019first, panageas2017gradient}, the set of initial conditions converging to strict saddles has Lebesgue measure zero, ensuring almost sure convergence to zero-energy satisfying models: $\lim_{N \to \infty} \rho_{\rm mult}(\alpha) = 0$.
    \item \textbf{Status of Separation:} While Theorem~\ref{thm:universal_contraction} establishes that Hinge trajectories converge to the LP polytope $Z$, proving an analytical lower bound $\lim_{N \to \infty} \rho_{\rm quad}(\alpha) \ge c(\alpha) > 0$ requires a standalone subcritical basin analysis (which cannot be inherited from Theorem 8 or Theorem 9). Consequently, Theorem 10 is classified as \textbf{Not Closed / Under Active Investigation}.
\end{enumerate}
\end{theorem}
...
Corollary 4B (LaSalle Vertex Confinement) & \textbf{Closed} (Strict Lyapunov on compact hypercube) \\
Theorem 5 (Hessian Factorization) & \textbf{Closed with rank/convexity conditions} ($V^T W V \succeq 0$) \\
Theorem 6 (Lipschitz and IEEE 754) & \textbf{Closed with IEEE conventions} ($L_\beta = \Theta(\beta)$, underflow thresholds) \\
Theorem 7B (Centripetal Contraction) & \textbf{Near Closed} ($\mathcal{E}_{\rm proj} \equiv Z$, normal cone conventions explicit) \\
Theorem 8 (Jensen Volume Bound) & \textbf{Closed as finite-dimensional bound} ($\mathbb{E}[\mu(Z)] \ge (5/6)^{\alpha N} > 0$) \\
Theorem 9 (Horn Linear Separation) & \textbf{Falsified / Abandoned} (Lema 9.1 falsified under unit fact; open for general DAGs) \\
Lemma 10.1 (Hypertree Overlap Bound) & \textbf{Partially Closed} ($\mathbb{E}[X] \le 9\alpha^2 = \mathcal{O}(1)$, $2\text{-core} = \emptyset$ a.a.s.) \\
Lemma 10.2 (Strict Saddle Subcritical) & \textbf{Closed conditionally under structural leaf hypothesis $H_{\rm leaf}$} \\
Theorem 10 (Subcritical 3-SAT Separation) & \textbf{Not Closed} (Multilinear avoidance closed; Hinge separation open) \\

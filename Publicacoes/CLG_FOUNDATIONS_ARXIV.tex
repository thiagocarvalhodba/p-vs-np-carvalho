...
\begin{lemma}[Lemma 10.1: Subcritical Hypertree Peeling and Overlap Bound]
\label{lem:peeling}
For random 3-SAT below the hypergraph percolation threshold $\alpha < \alpha_c = 1/6$:
\begin{enumerate}
    \item The expected number of clause pairs sharing $\ge 2$ variables is bounded by:
    \begin{equation}
    ...
    \item In the linear hyperforest regime ($|c \cap c'| \le 1$), the leaf-peeling algorithm $\mathcal{A}_{\rm peel}$ terminates, inducing an elimination ordering $\pi = (c_1, \dots, c_M)$ where each leaf clause $c_t$ contains at least two private variables of global degree 1 (appearing in no other clause of the formula). Flipping a private degree-1 variable of a violated leaf strictly reduces $E_{\rm disc}$ without violating any other clause, establishing the absence of discrete local minima with $E_{\rm disc} > 0$.
\end{enumerate}
\end{lemma}

\begin{lemma}[Lemma 10.2: Subcritical Strict Saddle Lemma via Spectral Tracelessness]
...
\begin{enumerate}
    ...
    \item For any violated clause $c$ at $x^*$, let $x_\ell$ be a private variable of global degree 1 guaranteed by Lemma~\ref{lem:peeling}, and let $x_p$ be an internal variable of $c$. Because $x_\ell$ appears in \textbf{no other clause} of the entire formula, the cross-derivative:
    \begin{equation}
    H_{\ell p} = \frac{\partial^2 \Phi_{\rm mult}}{\partial x_\ell \partial x_p}(x^*) = \frac{\sigma_\ell \sigma_p}{4}\left(\frac{1 - \sigma_k x^*_k}{2}\right) \ne 0
    \end{equation}
    ...
    \end{equation}
\end{enumerate}
Thus, every non-satisfying relative critical point in faces of dimension $\ge 2$ is unconditionally a strict saddle; flat saddles and degenerate positive semi-definite saddles are impossible.
\end{lemma}

\begin{theorem}[Theorem 10: Multilinear Saddle Avoidance and Subcritical Status]
\label{thm:subcritical_separation}
For the random 3-SAT ensemble $\mathcal{E}(N, \alpha)$ with $\alpha < 1/6$:
\begin{enumerate}
    \item Under the linear hyperforest structure, by Lemmata~\ref{lem:peeling} and \ref{lem:strict_saddle}, all non-satisfying relative critical points of $\Phi_{\rm mult}$ in faces of dimension $d \ge 2$ are strict saddles ($\lambda_{\min} < 0$), and no discrete positive local minima exist.
    ...
    \item \textbf{Status of Hinge Separation:} While Theorem~\ref{thm:universal_contraction} establishes that Hinge trajectories converge to the LP polytope $Z$, proving an analytical lower bound $\lim_{N \to \infty} \rho_{\rm quad}(\alpha) \ge c(\alpha) > 0$ requires a standalone subcritical basin analysis (which cannot be inherited from Theorem 8 or Theorem 9). The full dynamical separation in subcritical 3-SAT is therefore classified as \textbf{Open / Under Active Formalization}.
\end{enumerate}
\end{theorem}

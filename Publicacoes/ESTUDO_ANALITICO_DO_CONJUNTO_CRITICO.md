...
### Lema 10.5 (Convergência do PDS em Componentes Arbóreas — Fechado)

> **Lema 10.5.** Em uma componente arbórea finita, toda trajetória do fluxo projetado converge a um equilíbrio projetado de energia zero. A prova não usa mapas de impacto nem preservação de medida-zero.
>
> A ideia é tratar diretamente o PDS como o fluxo subgradiente da função estendida $F=\Phi+\delta_X$. Como $\Phi$ e o hipercubo $X$ são semialgébricos, $F$ é KL. A teoria de fluxos subgradientes KL fornece convergência de toda trajetória limitada. Resta excluir equilíbrios positivos: se $\Phi(x^*)>0$, existe cláusula violada; como a componente é arbórea, essa cláusula possui uma variável folha $\ell$. A contribuição da cláusula dá $\sigma_\ell\partial_\ell\Phi(x^*)<0$. Se $x_\ell^*$ é interior, o campo projetado tem componente estritamente não nula; se $x_\ell^*=-\sigma_\ell$, a única fronteira compatível com a violação, o campo projetado aponta estritamente para dentro. Logo nenhum equilíbrio projetado positivo existe. Portanto o limite tem energia zero e, pela extensão multilinear não negativa, existe um vértice da face limite com energia discreta zero. A demonstração completa está em `Publicacoes/CLG_T10_LEMA_10_5_SADDLE_AVOIDANCE.md`.

**Nota de auditoria:** esta formulação substitui a tentativa anterior de provar `saddle → bacia de medida zero` por uma afirmação mais forte: **não existem equilíbrios projetados de energia positiva em componentes arbóreas**. Isso também elimina a necessidade de controlar a invertibilidade dos mapas de impacto, que pode falhar para PDS na fronteira.

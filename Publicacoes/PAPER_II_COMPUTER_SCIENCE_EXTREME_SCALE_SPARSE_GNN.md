...
In this paper, we present the **Carvalho Extreme-Scale Sparse Diffusion Engine (Sparse-GNN)**, a hardware-agnostic neural framework for discrete graph partitioning that operates strictly in $\mathcal{O}(\\vert E \\vert)$ space and time complexity. By formulating continuous node partitioning as a discretized neural differential equation driven by sparse edge indexing (`index_add_`) and analytical Laplacian message passing, our architecture completely eliminates the requirement for dense matrix allocations or spatial attention tensors. 
...
| **Dense Graph Attention (GAT)** | Multi-Head Softmax Attention | $\mathcal{O}(\\vert E \\vert \cdot d + N^2)$ | $\mathcal{O}(N^2 \cdot H)$ attention map | OOM on consumer hardware at $N \ge 5,000$ |
| **Carvalho Sparse-GNN (Ours)** | Indexed Differential Diffusion | $\mathcal{O}(\\vert E \\vert)$ | $\mathcal{O}(\\vert E \\vert + N)$ linear buffers | **Sub-second ($0.79\text{s}$ at $N=10^4$)** |
...
2. **Strict $\mathcal{O}(\\vert E \\vert)$ Implementation Architecture**: We design an indexed neural diffusion layer utilizing PyTorch's atomic accumulation primitive (`index_add_`), achieving hardware cache locality, zero allocation overhead, and optimal vector SIMD execution.
...
| Instance ID | Graph Type | $\\vert V \\vert$ | $\\vert E \\vert$ | Greedy Cut (%) | Hybrid-GNN Cut (%) | **Sparse-GNN Cut (%)** | Sparse vs Greedy Gain |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
...
- **Evaluation Metrics:** Cut cardinality, cut fraction ($\\vert C \\vert / \\vert E \\vert$), execution wall-clock time (seconds), computational throughput (edges processed per second), dynamic memory consumption (megabytes).
...
| **Cut Edges Cardinality ($\\vert C \\vert$)**| 26,793 edges | **29,321 edges** | **+2,528 cut edges** |
| **Cut Ratio ($\\vert C \\vert / \\vert E \\vert$)** | $67.02\%$ | **$73.35\%$** | **+6.33 percentage points** |
...
| Tier | Graph Order ($N$) | Edge Count ($\vert E \vert$) | Density ($2\vert E \vert / N^2$) | Latency (s) | Throughput (Edges/s) | Peak RAM (MB) |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
...
The empirical exponent of the time scaling curve is $1.02 \approx 1.00$, confirming strict $\mathcal{O}(\\vert E \\vert)$ linear complexity in practice.
